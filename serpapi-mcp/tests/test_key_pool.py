...
async def pool():
    records = {
        "k1": _rec("k1", "SERP-a", status="active", remaining=900),
        "k2": _rec("k2", "SERP-b", status="active", remaining=800),
    }
...
async def test_concurrent_borrow_and_reload_and_success(pool):
    ...
    # 会被 reload 回退成 90/80，10 分差距 1 次借用盖不过，测不出分散
    tie_remaining = 20  # ratio=0.2（quota 100）→ 状态恒 active 不断言漂移
