"""SerpAPI MCP Server — multi-key search via SerpAPI (5 engines).

Env vars:
- REDIS_URL (必填): Redis 连接，key 池从这里读（如 redis://redis:6379/0）
- MCP_HOST / MCP_PORT (默认 127.0.0.1 / 9052)
- LOG_FORMAT: console|json
- PROMETHEUS_PORT (默认 9464)
- SERPAPI_QUOTA_DEFAULT (默认 100): 未设置 monthly_quota 时默认月配额
"""
import asyncio
import os
...
MCP_PORT = int(os.environ.get("MCP_PORT", "9052"))
REDIS_URL = os.environ.get("REDIS_URL", "")
QUOTA_DEFAULT = int(os.environ.get("SERPAPI_QUOTA_DEFAULT", "100"))
...
def _init_pool() -> KeyPool:
    ...
    _pool = KeyPool("serpapi", client, pubsub, quota_default=QUOTA_DEFAULT)
    return _pool
...
# 这里用宽异常保持对称；zabbix 无 except 直接裸导入，此处更稳)
try:
    from telemetry import init_telemetry
    init_telemetry("serpapi-mcp")
except Exception as exc:
    logger.warning("telemetry_init_failed", service="serpapi-mcp", error=str(exc))

mcp = FastMCP(
    "SerpAPI MCP",
    instructions=(
        "Search tools backed by SerpAPI with automatic API key rotation. "
        "Five engines: serpapi_google, serpapi_bing, serpapi_baidu, "
        "serpapi_duckduckgo, serpapi_ebay. All tools are read-only."
    ),
)
