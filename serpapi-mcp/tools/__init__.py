            ...
            try:
                ...
                if SEARCH_REQUESTS_TOTAL:
                    ...
                    SEARCH_REQUESTS_TOTAL.add(
                        1, attributes={"provider": "serpapi", "engine": tool_name, "status": status})
                return result
            except Exception:
                if SEARCH_REQUESTS_TOTAL:
                    SEARCH_REQUESTS_TOTAL.add(
                        1, attributes={"provider": "serpapi", "engine": tool_name, "status": "error"})
                raise
            finally:
                duration = time.monotonic() - start
                if SEARCH_REQUEST_DURATION:
                    SEARCH_REQUEST_DURATION.record(duration, attributes={"provider": "serpapi", "engine": tool_name})
        return wrapper
    return decorator
