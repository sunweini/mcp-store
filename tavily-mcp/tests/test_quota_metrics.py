...
def test_quota_level_mapping():
    ...
    try:
        ...
        assert fakes.ratio.adds[-1][1] == {"provider": "tavily", "level": QUOTA_LEVEL_WARNING}
        record_quota_metrics("tavily", {"lowest_ratio": 0.03, "lowest_remaining": 30,
                                        "pool_size": 2, "invalid_count": 0})
        assert fakes.ratio.adds[-1][1] == {"provider": "tavily", "level": QUOTA_LEVEL_CRITICAL}
        record_quota_metrics("tavily", {"lowest_ratio": 0.0, "lowest_remaining": 0,
                                        "pool_size": 2, "invalid_count": 0})
        assert fakes.ratio.adds[-1][1] == {"provider": "tavily", "level": QUOTA_LEVEL_EXHAUSTED}
        ...
        assert last == (1, {"provider": "tavily", "level": QUOTA_LEVEL_EXHAUSTED})
