...
- **span exception event 手动记录**（aliyun_client.py）：`start_as_current_span` 默认 `record_exception=True`——with 块未捕获异常退出时 SDK 自动 record_exception，自动事件 stacktrace 含 `raise ... from exc` 原始异常链（ConnectionError 消息带完整 URL query），绕过剥离防线。必须显式关掉（`record_exception=False, set_status_on_exception=False`），network_error 用 `add_event` 只写类型名 + 剥离后消息，其余路径保留 `record_exception`。`tests/test_aliyun_client.py` 用 InMemorySpanExporter 断言 event 属性无凭证无 `"?"`
