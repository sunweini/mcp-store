...
](#direct-http-server)Direct HTTP Server
...
](#host-and-origin-protection)Host and Origin Protection
...
](#gateway-routing-headers)Gateway Routing Headers
...
](#cors-for-browser-based-clients)CORS for Browser-Based Clients
...
](#sse-polling-for-long-running-operations)SSE Polling for Long-Running Operations
...
- When `event_store` is configured, the server stores all events (progress updates, results) with unique IDs

- Calling `ctx.close_sse_stream()` gracefully closes the HTTP connection

- The client automatically reconnects with a `Last-Event-ID` header
...
](#mounting-in-starlette)Mounting in Starlette
...
](#mounting-authenticated-servers)Mounting Authenticated Servers
...
-
**Forgetting to mount `.well-known` routes at root** - FastMCP cannot do this automatically when your server is mounted under a path prefix. You must explicitly mount well-known routes at the root level.

-
**Including mount prefix in both base_url AND mcp_path** - The mount prefix (like `/api`) should only be in `base_url`, not in `mcp_path`. Otherwise you’ll get double paths.
✅ **Correct:**
...
](#without-stateless-mode)Without Stateless Mode
...
](#enabling-stateless-mode)Enabling Stateless Mode
...
](#oauth-token-security)OAuth Token Security
...
- **Persistent network-accessible storage** for upstream tokens (wrapped in `FernetEncryptionWrapper` to encrypt sensitive data at rest)
...
](#running-fastmcp-as-a-linux-service)Running FastMCP as a Linux Service
...
](#mounting-under-a-path-prefix)Mounting Under a Path Prefix
...
](#hosting-your-server)Hosting Your Server
...
The key requirements are Python 3.10+ support and the ability to expose an HTTP port. Most providers will require you to package your server (requirements.txt, Dockerfile, etc.) according to their deployment format. For managed, zero-configuration deployment, see [Prefect Horizon](/deployment/prefect-horizon).[Running Your Server
Previous](/deployment/running-server)[Sandboxed Agents
Next](/deployment/sandboxed-agents)⌘I