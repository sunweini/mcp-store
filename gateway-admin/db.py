...
连接串从 MYSQL_URL 解析（mysql://user:pass@host:port/db）。admin 启动后首次
查询 /api/calls 时懒加载池；池断线 aiomysql 自动重连。

与 gateway-proxy/db.py 同款：proxy 写 calls 表，admin 读 calls 表。
"""
import os
