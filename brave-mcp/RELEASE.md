...
容器内端口 **9051**（仓库规范 9050-9500，见根 CLAUDE.md）。与 gateway-proxy
...
| `SEARCH_PROXY` | 空（直连） | **必配**（生产）：`http://10.16.12.12:7890`。生产网络 api.search.brave.com 直连不通（IPv4 被墙/IPv6 不通），必须走代理；本地开发可空。tavily/serpapi 直连通，不要给它们配此变量 |
...
**Key 管理**：key 存 Redis `search:keys:brave`，由 gateway-admin 前台
API Keys 页维护（编辑后自动 publish 热重载），**不在容器配 key env**。

## 工具清单

2 个只读工具（全部 `readOnlyHint=True`）：
...
```bash
curl -X POST http://<host>:9051/mcp \
...
### Unreleased
- 初始版本：2 个 tool（web_search/local_search），KeyPool 多 key 轮换 + 故障转移
