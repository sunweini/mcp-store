...
class FakeClient:
    ...
    def __init__(self, key, timeout=5.0, fail=None, fail_status=401, fail_exc=None):
        self.key = key
...
def _client_factory(made, fail=None, fail_status=401, fail_once=False, fail_exc=None):
    ...
    def _make(key, timeout):
        client = FakeClient(key, timeout=timeout,
                            ...
                            fail_status=fail_status, fail_exc=fail_exc)
        made.append(client)
        return client
    return _make
...
async def test_rate_limit_backoff_before_retry(monkeypatch, fake_pool):
    ...
    monkeypatch.setattr(web_mod.asyncio, "sleep", fake_sleep)
    made = []
    result = await brave_web_search(
        "q", pool=fake_pool,
        client_factory=_client_factory(made, fail="web_search", fail_status=429,
                                       fail_once=True))
    assert result["status"] == "ok"  # k1 429 → 退避 → k2 成功
    assert [c.key for c in made] == ["BSA-a", "BSA-b"]
    assert slept == [web_mod._RATE_LIMIT_BACKOFF]  # 恰好退避一次
...
async def test_non_rate_limit_no_backoff(monkeypatch, fake_pool):
    ...
    result = await brave_web_search(
        "q", pool=fake_pool,
        client_factory=_client_factory(made, fail="web_search", fail_status=401,
                                       fail_once=True))
    assert result["status"] == "ok"
    assert [c.key for c in made] == ["BSA-a", "BSA-b"]
...
class _FakeTimeout(Exception):
    ...
    曾因 kind or ErrorKind.EXHAUSTED 兜底把好 key 永久剔除——本组测试
    锁定「不写 key 状态」。
    """
...
async def test_retry_failover_timeout_on_second_key_keeps_second_key():
    ...
    def _make(key, timeout):
        client = FakeClient(key, timeout=timeout,
                            fail="web_search" if not made else None,
                            fail_exc=None if not made else _FakeTimeout())
        made.append(client)
        return client

    result = await brave_web_search("q", pool=pool, client_factory=_make)
    assert result["status"] == "error"
