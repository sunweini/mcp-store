...
async def _make_pool(records):
    pool = KeyPool("brave", FakeRedis(records), AsyncMock(), quota_default=1000)
    await pool.reload()
    return pool
...
async def test_health_snapshot_aggregates_pool(monkeypatch):
    ...
    records = {
        "k1": _rec("k1", "brv-a", remaining=50),     # ratio 0.05 → critical
        "k2": _rec("k2", "brv-b", remaining=80),     # ratio 0.08 → warning
        "k3": _rec("k3", "brv-c", remaining=None),   # unknown → 不参与
        "k4": _rec("k4", "brv-d", status="invalid"),  # invalid，无 remaining
    }
...
async def test_reload_emits_metrics(monkeypatch):
    ...
    records = {
        "k1": _rec("k1", "brv-a", remaining=900),
        "k2": _rec("k2", "brv-b", remaining=800),
    }
    pool = await _make_pool(records)
    assert calls[-1][0] == "brave"
...
async def test_rate_limit_does_not_emit_metrics(monkeypatch):
    ...
    monkeypatch.setattr("key_pool.record_quota_metrics",
                        lambda provider, snap: calls.append(snap))
    records = {"k1": _rec("k1", "brv-a", remaining=900)}
    pool = await _make_pool(records)
    calls.clear()
...
def test_quota_level_mapping():
    ...
    try:
        record_quota_metrics("brave", {"lowest_ratio": 0.08, "lowest_remaining": 80,
                                        "pool_size": 2, "invalid_count": 1})
        assert fakes.ratio.adds[-1][1] == {"provider": "brave", "level": QUOTA_LEVEL_WARNING}
        record_quota_metrics("brave", {"lowest_ratio": 0.03, "lowest_remaining": 30,
                                        "pool_size": 2, "invalid_count": 0})
        assert fakes.ratio.adds[-1][1] == {"provider": "brave", "level": QUOTA_LEVEL_CRITICAL}
        record_quota_metrics("brave", {"lowest_ratio": 0.0, "lowest_remaining": 0,
                                        "pool_size": 2, "invalid_count": 0})
        assert fakes.ratio.adds[-1][1] == {"provider": "brave", "level": QUOTA_LEVEL_EXHAUSTED}
        # remaining 未知（None）但 ratio None → 不发 ratio（不编造档位）
        record_quota_metrics("brave", {"lowest_ratio": None, "lowest_remaining": None,
                                        "pool_size": 2, "invalid_count": 0})
        # ratio None 但 remaining==0 → 兜底 exhausted
        record_quota_metrics("brave", {"lowest_ratio": None, "lowest_remaining": 0,
                                        "pool_size": 2, "invalid_count": 0})
        assert fakes.ratio.adds[-1][1] == {"provider": "brave", "level": QUOTA_LEVEL_EXHAUSTED}
        # remaining/ratio 正常（>=10%）→ 只发 size/invalid/remaining，不发 ratio
        record_quota_metrics("brave", {"lowest_ratio": 0.5, "lowest_remaining": 500,
                                        "pool_size": 2, "invalid_count": 0})
        last = fakes.ratio.adds[-1]
        assert last == (1, {"provider": "brave", "level": QUOTA_LEVEL_EXHAUSTED})
...
def test_metrics_none_guard(monkeypatch):
    ...
    try:
        record_quota_metrics("brave", {"lowest_ratio": 0.01, "lowest_remaining": 5,
